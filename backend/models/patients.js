...
const getPatientById = async (db, patientId) => {
  const { rows } = await db.query(
    `SELECT patients.patient_case, patients.location_found, patients.date_admitted, patients.release_date, patients.is_released, patients.story, patients.is_archived, species.name, COALESCE(patients.image, species.image) AS image FROM patients
    JOIN species ON patients.species_id = species.id
    WHERE patients.id = $1`,
    [patientId]
  );
...
const updatePatientInformation = async (db, patientId, patientDetails, conditionIds, treatmentIds) => {
  try {
    ...
    // collects conditions and treatments and images to compare with updates
