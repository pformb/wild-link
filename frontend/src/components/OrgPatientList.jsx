...
const OrgPatientList = () => {
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Button variant="contained" color="primary" sx={{ mb: 4, ml: 10 }} onClick={() => goToNewForm(orgId)}>
        Add New Patient
      </Button>
      ...
      <Grid
        container
        spacing={2}
        sx={{ width: "75%", mx: "auto", boxShadow: 3, p: 3, display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(450px, 1fr))", gap: 2, alignItems: "flex-start", justifyContent: "center"}} >
        {patients ? (
          patients.filter(patient => (selectedTab === 0 ? !patient.is_released : patient.is_released)).map((patient, index) => (
            <Grid
              item
              sx={{ width: 450, mb: 2, mx: "auto" }}
              key={index}
            >
              <Card sx={{ width: "100%", maxWidth: 450, display: "flex", flexDirection: "row", alignItems: "center", p: 2, boxShadow: 2, mb: 2 }}>
